...
func testUseServiceAsync(f *finder.FinderManager) {
	handler := new(ServiceChangedHandle)
	var subscribe = common.ServiceSubscribeItem{ServiceName: "test0803", ApiVersion: "1.0"}
	serviceList, err := f.ServiceFinder.UseAndSubscribeService([]common.ServiceSubscribeItem{subscribe}, handler)
	if err != nil {
		fmt.Println(err)
	} else {
		for _, s := range serviceList {
